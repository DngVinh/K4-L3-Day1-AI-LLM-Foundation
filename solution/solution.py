...
def call_openai(
    ...
    # Chỉ truyền base_url khi được cấu hình để vẫn dùng được endpoint mặc định
    # của OpenAI, đồng thời hỗ trợ các dịch vụ tương thích OpenAI.
...
def compare_models(prompt: str) -> dict:
    ...
    # Ước lượng thô: 0,75 từ tương đương khoảng 1 token.
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Model lạ, encoding chưa tải được hoặc tiktoken gặp lỗi: dùng ước lượng.
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    default_pricing = PRICING_PER_1K_TOKENS.get(
...
def run_assistant(
    ...
    while True:
        # Kiểm tra giới hạn trước khi yêu cầu lượt nhập tiếp theo.
        if max_turns is not None and num_turns >= max_turns:
            break
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def compact(value: object, limit: int = 40) -> str:
        text = " ".join(str(value).split())
        return text if len(text) <= limit else f"{text[: limit - 3]}..."

    rows = [
        [
            compact(result.get("prompt", "")),
            compact(result.get("gpt4o_response", "")),
            compact(result.get("mini_response", "")),
            compact(result.get("gpt4o_latency", "")),
            compact(result.get("mini_latency", "")),
        ]
